...
From Table 1.2, we can see that some covariates are insignificant. Hence, we conducted stepwise selection with both forward and backward direction and AIC as  selection criteria @stepAIC. From the AIC stepwise selection, we obtained the stepwise model for main effects. 
...
To begin with, let's assign each feature with a variable name \ref{table: data_dictionary}.
...
But there are still some useless covariates in the model. To simplify the model, a backward step-wise selection was used to the square of numerical model. The parameters and their significance of selected model are shown in the table \ref{table: reduced_squared}.
...
The best model to predict heart disease is model 2, which contains SexM, ChestPainTypeATA ChestPainTypeNAP, ChestPainTypeTA, Cholesterol, FastingBS, ExerciseAnginaY, ST\_slopeFlat, ST\_slopeUp, Age^2, Oldpeak ^2, Cholesterol ^2. The formula of the model 2 is displayed below.
...
(7) The odds ratio of having a heart disease is positively related to the quadratic of age.  The odds ratio will increase by $2.2 \times 10^{-4}$ when the squared age increase by one unit.
